...
fn filter_batch_impl(sections: &[TaskSection], trailing: &str) -> String {
    ...
    for section in sections {
        if section.failed {
            ...
            let filtered = filter_section_content(&globally_filtered_content, &section.task_type);
...
fn detect_task_type_from_name(task_name: &str) -> TaskType {
    ...
    if name == "test" {
        TaskType::Test
    } else if name == "integrationTest" || name == "componentTest" {
        TaskType::IntegrationTest
    } else if name.starts_with("detekt") {
        TaskType::Detekt
    } else if name.starts_with("projectHealth") {
        TaskType::Health
    } else if name == "compileKotlin" || name == "compileTestKotlin" || name.ends_with("Classes") {
        TaskType::Compile
    } else if name.contains("Proto") || name == "buildProtos" || name == "generateProtos" {
        TaskType::Proto
