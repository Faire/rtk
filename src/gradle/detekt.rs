...
lazy_static! {
    ...
    /// Detekt-specific noise compiled into a single RegexSet.
    static ref DETEKT_NOISE: RegexSet = RegexSet::new([
        r"^Loading baseline",
        r"^Comparing against baseline",
        r"^Progress: analyzing",
    ]).unwrap();
}
...
pub fn filter_detekt(input: &str) -> String {
    ...
    for line in input.lines() {
        ...
        if DETEKT_NOISE.is_match(trimmed) {
            continue;
        }
